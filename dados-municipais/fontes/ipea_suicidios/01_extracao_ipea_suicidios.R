# -*- coding: UTF-8 -*-
# ------------------------------------------------------------------------------
# SCRIPT: 01_extracao_ipea_suicidios.R
# FONTE: Ipea - Atlas da Violencia (obitos por suicidio do SIM/DATASUS), serie
#        AVIOL12_SUICID da API OData do Ipeadata; populacao ESTIMA_PO (IBGE via
#        Ipeadata) como denominador da taxa
# OBJETIVO: Baixar e preservar as duas series exatamente como a API devolve
#           (todos os niveis territoriais), sem transformar.
# COBERTURA: Brasil, todos os municipios; AVIOL12_SUICID desde 1980 ate o
#            ultimo ano divulgado; ESTIMA_PO desde 1992
# PERIODICIDADE: anual
# ENDPOINT: https://www.ipeadata.gov.br/api/odata4/ValoresSerie(SERCODIGO='<serie>')
# SAIDAS: dados/brutos/ipea_suicidios/ipeadata_<serie>.csv
#         dados/brutos/ipea_suicidios/ipeadata_<serie>_metadados.csv
# COMO EXECUTAR: Rscript fontes/ipea_suicidios/01_extracao_ipea_suicidios.R
# ------------------------------------------------------------------------------
...
#   AVIOL12_SUICID - numero de suicidios por municipio de residencia (SIM;
#                    CID-10 X60-X84; CID-9 E950-E959 ate 1995)
#   ESTIMA_PO      - populacao residente estimada em 1o de julho (denominador
#                    da taxa; baixada aqui para a fonte nao depender de outra)
SERIES <- c("AVIOL12_SUICID", "ESTIMA_PO")
...
baixar_serie_ipeadata <- function(serie, dir_saida) {
  ...
  # valores ja divulgados (atualizacoes do SIM e das estimativas). Por isso o
